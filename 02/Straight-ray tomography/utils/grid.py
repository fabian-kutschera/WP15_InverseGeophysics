...
class grid(object):
    ...
    def attach_field(self, name, values, verbose=True):
        ...
        assert np.array_equal(
            np.asarray(values.reshape(self.npoints).shape), self.npoints
        )
            ...
            for field in self.data:
                if smoothing > 0:
                    interpolator = RectBivariateSpline(
                        x, y, gaussian_filter(self.data[field], smoothing)
                    )
                else:
            ...
            for field in self.data:
                if smoothing > 0:
                    interpolator = RegularGridInterpolator(
                        (x, y, z), gaussian_filter(self.data[field], smoothing)
                    )
                else:
    ...
    def get_gaussian_prior(self, smoothing_length):
        ...
        for idx in range(0, nx):
            for idy in range(0, ny):
                i = ny * idx + idy
                scale = K[
                    max(s - idx, 0) : min(2 * s + 1, nx + s - idx),
                    max(s - idy, 0) : min(2 * s + 1, ny + s - idy),
                ].sum()
