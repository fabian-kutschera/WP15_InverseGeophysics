...
def butterworth_lp(signal, t, cutoff, npoles=4, plot=False):
    ...
    # - Determine the poles. ---------------------------------------------------
    for k in np.arange(0, npoles):
        s[k] = wc * np.exp(1j * np.pi * (2.0 * (k + 1) + n - 1) / (2.0 * n))
...
def butterworth_hp(signal, t, cutoff, npoles=4, plot=False):
    ...
    # - Determine the poles. ---------------------------------------------------
    for k in np.arange(0, npoles):
        s[k] = -wc * np.exp(1j * np.pi * (n + 1.0 - 2.0 * (k + 1.0)) / (2.0 * n))
    ...
    for i in range(npoles):
        for k in range(npoles):
            if not i == k:
                Pi[i] = Pi[i] * (1.0 - s[i] / s[k])

    Pi = Pi / (wc ** n)

    for k in range(npoles):
        h -= s[k] ** n * np.exp(s[k] * t) / Pi[k]
    for k in range(1, npoles - 1):
        h[k] = (h[k + 1] - h[k]) / (t[1] - t[0])
    h[0] = h[0] - np.sum(h)
